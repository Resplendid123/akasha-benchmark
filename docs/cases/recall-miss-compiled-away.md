...
早期假设认为「**多跳可能偏高**，因为实体被物化成独立 artifact 并
...
（这是对早期假设的实测更正，原文的机制描述没错 —— 编译器确有合并能力，
...
排查这一条走了六跳、跨五张表，全靠手写 SQL。平台血缘视图用于重现这条链路。

另外「**原文 vs 编译产物 diff**」应当是一等视图：本案的根因只有把两者并排才看得见。
原文基线从整体上量这个效应，而逐样本 diff 能直接指出丢了哪个词 ——
