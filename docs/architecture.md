...
阶段在后端进程里执行，一个任务一条线程；编译任务最多同时运行 3 个，其他同名阶段同时只运行 1 个。`execute()` 统一处理执行与链路校验，
...
- **继续**使用原任务保存的实际参数与产物 ID，由各阶段决定重算或跳过：编译并发导入文档，导入失败时继续任务只补传未导入文档；远端编译失败时，从已保存 Run 的逐页诊断读取失败页并调用 Akasha `admin/retry-pages` 批量重试，不重新编译整个 Space；因主动暂停而标为 `manual_cancelled` 的跳过页也会重试。若 Run 在建立逐页记录前即被取消，才重新提交首次编译。重启后 Run 仍活动则接管等待，Run 已完成则只补做质量检查。查询跳过已有响应的样本，确定性评测按数据集重算，Judge 按指标跳过已有判定，归因跳过已有结果的样本。
...
- **编译完整性**：`missingChunk`、`missingEmbedding`、`missingSource`、`stalePageCount` 四项计数必须全部取得且为 0，整次编译才算成功。若远端逐页进度明确显示至少一篇成功，则失败记录仍可降级供查询层使用，并提示结果不完整；没有成功篇数证据时仍拒绝查询。
