...
function NewEval({
  ...
  // 默认勾选「对所选每一组都算得出来」的那些。
  useEffect(() => {
    if (metrics.data && selected.length === 0) {
      setSelected(metrics.data.computable_for_all.filter((n) => n !== 'faithfulness'))
    }
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      <div className="metric-grid">
        {metrics.data.definitions.map((definition) => {
    ...
    <div className="panel" style={{ marginTop: 14 }}>
      ...
      {data.judge.total > 0 && (
        <div className={`note${data.judge.failure_rate > 0.1 ? ' bad' : ' plain'}`}>
          <strong>faithfulness {data.judge.mean?.toFixed(4) ?? '—'}</strong> · 已评分{' '}
          {data.judge.scored}/{data.judge.total}，失败率 {percent(data.judge.failure_rate)}。
...
/** 全样本 vs 仅 knowledge 并排。差值就是生成端拒答的规模。 */
